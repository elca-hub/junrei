...
import type InewWindow from "../interfaces/window/new"
...
function searchMap(query: string) {
  ...
  service.findPlaceFromQuery(request, (results, status) => {
    if (status === google.maps.places.PlacesServiceStatus.OK && results) {
      ...
      if (centerViewResult.geometry?.location === undefined)
        throw new GoogleMapsError("location is undefined")

      map.setCenter(centerViewResult.geometry.location)
      const ZOOM_LEVEL = 18
      map.setZoom(ZOOM_LEVEL)

      const placeIdEle = document.getElementById(
        elementIdObj.placeId,
      ) as HTMLInputElement

      if (placeIdEle === null)
        throw new NoHTMLElementError(elementIdObj.placeId)

      if (centerViewResult.place_id === undefined)
        throw new GoogleMapsError("place_id is undefined")
...
function initMap() {
  ...
  const formTextEle = document.getElementById(
    elementIdObj.searchSpot,
  ) as HTMLInputElement

  if (formTextEle === null)
    throw new NoHTMLElementError(elementIdObj.searchSpot)
