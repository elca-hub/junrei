...
import { SwitchIsAchievedOutput, type SwitchIsAchievedInput } from "../../../dto/spots/switchIsAchievedDto";
...
export default class SwitchIsAchievedUsecase implements IUsecase<SwitchIsAchievedInput, Promise<SwitchIsAchievedViewModel>> {
    private readonly junreiApi: JunreiApi;
    private readonly switchIsAchievedPresenter: SwitchIsAchievedPresenter;

    constructor(switchIsAchievedPresenter: SwitchIsAchievedPresenter, junreiApi: JunreiApi) {
        this.junreiApi = junreiApi;

        this.switchIsAchievedPresenter = switchIsAchievedPresenter;
    }

    public async execute(input: SwitchIsAchievedInput): Promise<SwitchIsAchievedViewModel> {
        const spot = new Spot(
            input.getSpot().id,
        ...
        if (apiData.status !== successStatusCode) 
            return this.switchIsAchievedPresenter.fail(new SwitchIsAchievedOutput(apiData.message))
        else
            return this.switchIsAchievedPresenter.complete();
    }
}
